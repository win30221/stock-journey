...
const fmtPerShareNumber = value => value == null ? '—' : perShare.format(Number(value));
...
function calculateProjectedAnnualDividends({ transactions, marketCaches, asOfDate, requiredThroughDate = asOfDate }) {
...
function dividendKey(row) { return row.id || [row.exDate||'',row.paymentDate||'',row.announcementDate||''].join('|'); }
...
// Source: js/domain/trend.js
function calculateTrendHistory({ transactions, marketCaches, dateBasis, asOfDate }) {
...
function aggregateTrendMonths(daily) {
...
function memoizeLatest(compute) {
...
// Source: js/domain/transaction-validation.js
...
function validateTransactionFields(row, { acquisitionTypes = Object.keys(ACQUISITIONS) } = {}) {
...
// Source: js/components/dialog.js
let activeDialogCleanup = null;

function clearDialogFocus({ restoreFocus = false } = {}) {
  activeDialogCleanup?.(restoreFocus);
  activeDialogCleanup = null;
}

// Keep custom dialogs usable with a keyboard, including after an app rerender.
function bindDialogFocus(dialog, { onClose, initialFocus, returnFocus } = {}) {
...
function normaliseProjectionSetting(value, fallback, minimum, maximum) {
  const number = Number(value);
  return Number.isFinite(number) ? Math.max(minimum, Math.min(maximum, number)) : fallback;
}

function normaliseGainMilestoneInterval(value) {
  const parsed = Math.round(Number(value));
  return Number.isFinite(parsed) ? Math.max(10000, Math.min(100000000, parsed)) : 1000000;
}

function normaliseTrendTooltipEventLimit(value) {
  const parsed = Math.floor(Number(value));
  return Number.isFinite(parsed) ? Math.max(1, Math.min(20, parsed)) : 3;
}

function normaliseTrendEventMarkerSettings(source) {
  return Object.fromEntries(TREND_EVENT_MARKER_SETTINGS.map(({ id }) => [id, source?.[id] ?? true]));
}

function normaliseSettings(source, { asOfMonth, resetMarketSync = false } = {}) {
...
function createSettingsStore({ repository, initialSettings = createDefaultSettings() }) {
...
// Source: js/app/portfolio-model.js
/**
...
function createAutosave(write, { delay = 500, onError = () => {} } = {}) {
