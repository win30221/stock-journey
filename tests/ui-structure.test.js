...
test('manual transaction stock field supports accessible Chinese autocomplete', () => {
  const source = fs.readFileSync(path.resolve(__dirname, '..', 'app.js'), 'utf8');
  const searchSource = fs.readFileSync(path.resolve(__dirname, '..', 'js/app/stock-search.js'), 'utf8');
  ...
  assert.match(searchSource, /fetchData\('TaiwanStockInfo'\)/);
  assert.match(searchSource, /event\.key\s*===\s*'ArrowDown'/);
  assert.match(searchSource, /resolve\(query\)\s*\{\s*return resolveStockQuery\(stockCatalog,\s*query\)/);
  ...
  assert.match(styles, /\.transaction-field\s*\{[^}]*font-size:\s*\.875rem[^}]*font-weight:\s*700/, 'autocomplete field should use readable form-label typography');
});
...
test('mobile navigation opens as an accessible side drawer', () => {
  ...
  assert.match(source, /panel\.inert=isMobile\(\)&&!open/);
...
test('retirement results require an explicitly confirmed birth month at the moment of value', () => {
  const source = fs.readFileSync(path.resolve(__dirname, '..', 'app.js'), 'utf8');
  const settingsSource = fs.readFileSync(path.resolve(__dirname, '..', 'js/app/settings.js'), 'utf8');
  const styles = fs.readFileSync(path.resolve(__dirname, '..', 'styles.css'), 'utf8');

  assert.match(settingsSource, /retirementBirthMonthConfirmed:\s*false/);
...
test('completion notice survives incidental rerenders and clears on navigation', () => {
  ...
  assert.match(source, /if\(failures\.length\|\|!automatic\)toast\(syncMessage\)/, 'successful auto-sync should stay quiet instead of competing with the completion announcement');
});
...
test('retirement calculator connects portfolio, budget, projection assumptions and accessible results', () => {
  ...
  assert.match(source, /calculatePortfolioSnapshot/);
  assert.match(source, /function dividendForecast\(\) \{ return portfolioSnapshot\(\)\.dividendForecast; \}/);
