...
const source = buildBundle().replace(
  ...
  `\nglobalThis.__marketTest = {
    ...
    setPausedUntil(value) { settingsStore.replace({ ...settings, marketAutoSyncPausedUntil: value }); },
  };`,
);
...
assert.equal(market.marketSyncPlan('0050', '2026-08-31').dividendStart, '2025-01-02', '交易日早於股息回補窗時，股息快照也要包含全部持有期間');
